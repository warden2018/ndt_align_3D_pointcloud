...
void ROSMultiLidarCalibratorApp::PointsCallback(const sensor_msgs::PointCloud2::ConstPtr &in_parent_cloud_msg,
	...
	Eigen::Quaterniond eigen_quat(current_guess_.block<3,3>(0,0).cast<double>());
	//Eigen::Quaterniond eigen_quat(0,0,0,1);
	Eigen::Vector3d eigen_trans(current_guess_.block<3,1>(0,3).cast<double>());
	tf::Quaternion tf_quat;
	tf::Vector3 tf_trans;
	tf::quaternionEigenToTF(eigen_quat, tf_quat);
	tf::vectorEigenToTF(eigen_trans, tf_trans);
	std::string new_frame("calibrated_frame");
	tf::StampedTransform corrected_trans(tf::Transform(tf_quat, tf_trans), ros::Time::now(), parent_frame_, new_frame);
	corrected_tf_broadcaster.sendTransform(corrected_trans);
...
void ROSMultiLidarCalibratorApp::InitializeROSIo(ros::NodeHandle &in_private_handle){
	...
	in_private_handle.param<std::string>("init_param_file", init_param_file, "extrinsics");
	ROS_INFO("[%s] Read init param file from : %s",__APP_NAME__, init_param_file.c_str());
	...
	//init current guess 
	Eigen::Matrix4d *init_guess = new Eigen::Matrix4d;
	load_transformation_matrix_from_file(init_param_file,init_guess);
	current_guess_ = init_guess->matrix().cast <float> ();
}
...
bool ROSMultiLidarCalibratorApp::load_transformation_matrix_from_file(
    const std::string &file_name, Eigen::Matrix4d *transformation_matrix) {
  //ROS_INFO_STREAM("load_transformation_matrix_from_file is called; file name: "<< file_name.c_str());
  try {
    YAML::Node config = YAML::LoadFile(file_name);
    if (!config) {
      ROS_WARN("Open TransformationMatrix File: %s failed.",file_name.c_str());
      return false;
    }
    if (!config["transform"]) {
      ROS_WARN("Open TransformationMatrix File: %s has no transform.",file_name.c_str());
      return false;
    }
    //fill translation
    if (config["transform"]["translation"]) {
      ...
      ROS_WARN("TransformationMatrix File: %s has no transform:translation.",file_name.c_str());
      return false;
    }
    // fill rotation
    if (config["transform"]["rotation"]) {
      ...
      ROS_WARN("TransformationMatrix File: %s has no transform:rotation.",file_name.c_str());
      return false;
    }
  } catch (const YAML::Exception &e) {
    ROS_ERROR("%s load failed. error: %s",file_name.c_str(),e.what());
    ROS_ERROR("Please ensure param file is exist or format is correct");
    return false;
  }
